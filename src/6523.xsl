...
<xsl:stylesheet version="2.0"
	...
	<xsl:template name="code.iso.6523">
		...
		<xsl:choose>
			...
			<xsl:when test="$myparam.upper='0172'"><xsl:value-of select="$myparam"/> (Project Group "Lists of Properties" (PROLIST(r)))</xsl:when>
			...
			<xsl:when test="$myparam.upper='0197'"><xsl:value-of select="$myparam"/> (Name not known)</xsl:when>
