...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <!-- Begin: Jan Thiele -->
    <xsl:template name="identifier.7143">
        <xsl:if test="@listID | @schemeID">
			<xsl:variable name="listID_JT">
				<xsl:call-template name="code.UNTDID.7143">
					<xsl:with-param name="myparam" select="(@listID, @schemeID)[1]"/>			
				</xsl:call-template>
			</xsl:variable>	  
            <xsl:attribute name="scheme_identifier" select="(@listID_JT, @schemeID)[1]" />
        </xsl:if>
   ...
   <!-- Begin: Jan Thiele -->
   <xsl:template name="identifier.EAS">
		<xsl:if test="@listID | @schemeID">
			<xsl:variable name="listID_JT">
   ...
   <!-- Begin: Jan Thiele -->
   <xsl:template name="identifier.6523">
		<xsl:if test="@listID | @schemeID">
			<xsl:variable name="listID_JT">
   ...
   <!-- End: Jan Thiele -->        
	...
	<!-- Begin: Jan Thiele -->
	<xsl:template name="identifier.1153">
		<xsl:param name="schemeID" as="element()?"/>
		<xsl:if test="@schemeID">
			<xsl:variable name="listID_JT">
